...
void run_hotspot_gpu(cl_context context, int col, int row, int total_iterations, int num_iterations, int blockCols, int blockRows, int borderCols, int borderRows, float *TempCPU, float *PowerCPU, char *outputfile) {
    ...
    // 1. set up kernel
        ...
        cl_hotspot_program = cl_compileProgram((char *)"./xclbin/hotspot.hw_emu.xilinx_aws-vu9p-f1-04261818_dynamic_5_0.xclbin",NULL);
	       
        hotspot_kernel = clCreateKernel(cl_hotspot_program, "hotspot", &status);
...
////////////////////////////////////////////////////////////////////////
	...
	global_work[0] = BLOCK_SIZE * blockCols;
	global_work[1] =  BLOCK_SIZE * blockRows;
	size_t local_work_size[2];
 	local_work_size[0] = BLOCK_SIZE;
 	local_work_size[1] = BLOCK_SIZE;
	printf("local:%d",local_work_size[0]);
	printf("global:%d",global_work[0]);
//////////////////////////////////////////////////////////////////////// 
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////READ STARTS/////
    ...
    for (t = 0; t < total_iterations; t += num_iterations) {
		...
		error = clEnqueueNDRangeKernel(command_queue, hotspot_kernel, 2, 0, global_work,local_work_size, 0, NULL, &kernelEvent);
		//error = clEnqueueTask(command_queue,hotspot_kernel,NULL,NULL,&kernelEvent);
    ...
    cl_float *MatrixOut = (cl_float *) clEnqueueMapBuffer(command_queue, MatrixTemp[src], CL_TRUE, CL_MAP_READ, 0, sizeof(float) * size, 0, NULL, NULL, &error);
    
    // Write final output to output file
    writeoutput(MatrixOut, row, col, outputfile);
    
    error = clEnqueueUnmapMemObject(command_queue, MatrixTemp[src], (void *) MatrixOut, 0, NULL, NULL);
