...
void run_hotspot_gpu(cl_context context, int col, int row, int total_iterations, int num_iterations, int blockCols, int blockRows, int borderCols, int borderRows, float *TempCPU, float *PowerCPU, char *outputfile) {
    ...
    // 1. set up kernel
        ...
        cl_hotspot_program = cl_compileProgram((char *)"./xclbin/hotspot.sw_emu.xilinx_aws-vu9p-f1-04261818_dynamic_5_0.xclbin",NULL);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////READ STARTS/////
	...
	// 3. send arguments to device
		...
		cl_errChk(argchk,"ERROR in Setting hotspot_read kernel args",true);
	...
	// 3. send arguments to device	
		...
		cl_errChk(argchk,"ERROR in Setting hotspot_compute kernel args",true);
	...
	// 3. send arguments to device	
		...
		cl_errChk(argchk,"ERROR in Setting hotspot_write kernel args",true);
    ...
    //cl_float *MatrixOut = (cl_float *) clEnqueueMapBuffer(command_queue, MatrixTemp[src], CL_TRUE, CL_MAP_READ, 0, sizeof(float) * size, 0, NULL, NULL, &error);
     cl_float *MatrixOut = (float*) malloc(sizeof(float)*size);
      error = clEnqueueReadBuffer(command_queue2,  MatrixTemp[src], 1, 0, sizeof(float) * size, MatrixOut, 0,NULL,&kernelEvent2);
