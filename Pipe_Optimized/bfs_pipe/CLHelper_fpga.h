...
static cl_command_queue commandQueue1 = NULL;  
static cl_command_queue commandQueue2 = NULL;
//static cl_command_queue commandQueue3 = NULL;
...
char kernel_file[100]  = "kernel_bfs.cl";
int total_kernels = 3;
string kernel_names[3] = {"BFS_Reader", "BFS_CU", "BFS_store"};
...
void _clInit()
{
    ...
    //printf("Selected Platform = %s",targetPlatform);
    ...
    oclHandles.context = clCreateContextFromType(cprops,CL_DEVICE_TYPE_ALL,NULL,NULL,&resultCL);
   ...
   //--cambine-4: Create an OpenCL command queue    
    commandQueue = clCreateCommandQueue(oclHandles.context,oclHandles.devices[DEVICE_ID_INUSED],CL_QUEUE_PROFILING_ENABLE,&resultCL);

    if ((resultCL != CL_SUCCESS) || (commandQueue == NULL))
        throw(string("InitCL()::Creating Command Queue. (clCreateCommandQueue)"));
   
    commandQueue1 = clCreateCommandQueue(oclHandles.context,oclHandles.devices[DEVICE_ID_INUSED],CL_QUEUE_PROFILING_ENABLE,&resultCL);

    if ((resultCL != CL_SUCCESS) || (commandQueue1 == NULL))
        throw(string("InitCL()::Creating Command Queue. (clCreateCommandQueue)"));
    
    commandQueue2 = clCreateCommandQueue(oclHandles.context,oclHandles.devices[DEVICE_ID_INUSED],CL_QUEUE_PROFILING_ENABLE,&resultCL);

    if ((resultCL != CL_SUCCESS) || (commandQueue2 == NULL))
        throw(string("InitCL()::Creating Command Queue. (clCreateCommandQueue)"));

	 /*commandQueue3 = clCreateCommandQueue(oclHandles.context,oclHandles.devices[DEVICE_ID_INUSED],CL_QUEUE_PROFILING_ENABLE,&resultCL);

    if ((resultCL != CL_SUCCESS) || (commandQueue3 == NULL))
        throw(string("InitCL()::Creating Command Queue. (clCreateCommandQueue)"));*/
...
void _clRelease()
{
    ...
    {
        cl_int resultCL = clReleaseCommandQueue(commandQueue);
        if (resultCL != CL_SUCCESS)
        {
    ...
    {
        cl_int resultCL = clReleaseCommandQueue(commandQueue1);
        if (resultCL != CL_SUCCESS)
        {
            cerr << "ReleaseCL()::Error: In clReleaseCommandQueue" << endl;
            errorFlag = true;
    ...
    {
        cl_int resultCL = clReleaseCommandQueue(commandQueue2);
        if (resultCL != CL_SUCCESS)
        {
            cerr << "ReleaseCL()::Error: In clReleaseCommandQueue" << endl;
            errorFlag = true;
 ...
 /*if (commandQueue3 != NULL)
    {
        cl_int resultCL = clReleaseCommandQueue(commandQueue3);
        if (resultCL != CL_SUCCESS)
        {
            cerr << "ReleaseCL()::Error: In clReleaseCommandQueue" << endl;
            errorFlag = true;
...
void _clMemcpyH2D(cl_mem d_mem, int size, const void *h_mem_ptr) throw(string){
	oclHandles.cl_status = clEnqueueWriteBuffer(commandQueue, d_mem, CL_TRUE, 0, size, h_mem_ptr, 0, NULL, NULL);
	#ifdef ERRMSG
	if(oclHandles.cl_status != CL_SUCCESS)
...
cl_mem _clCreateAndCpyPinnedMem(int size, float* h_mem_source) throw(string){
	...
	#endif
	//----------
	h_mem_pinned = (cl_float *)clEnqueueMapBuffer(commandQueue, d_mem_pinned, CL_TRUE,CL_MAP_WRITE, 0, size, 0, NULL,NULL,  &oclHandles.cl_status);
	#ifdef ERRMSG
	if(oclHandles.cl_status != CL_SUCCESS)
	...
	//----------
	oclHandles.cl_status = clEnqueueWriteBuffer(commandQueue, d_mem,CL_TRUE, 0, size, h_mem_pinned,0, NULL, NULL);
	#ifdef ERRMSG
	if(oclHandles.cl_status != CL_SUCCESS)
...
void _clMemcpyD2H(cl_mem d_mem, int size, void * h_mem) throw(string){
	oclHandles.cl_status = clEnqueueReadBuffer(commandQueue2, d_mem, CL_TRUE, 0, size, h_mem, 0,0,0);
	#ifdef ERRMSG
...
void _clFinish(int kernel_id) throw(string){
	oclHandles.cl_status = clFinish(commandQueue);	
	#ifdef ERRMSG
...
void _clInvokeKernel(int kernel_id, int work_items, int work_group_size) throw(string){
	...
	oclHandles.cl_status = clEnqueueNDRangeKernel(commandQueue, oclHandles.kernel[kernel_id], work_dim, 0,global_work_size, local_work_size, 0 , 0, &e[kernel_id] );	
	#ifdef ERRMSG
