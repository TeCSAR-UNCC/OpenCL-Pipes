...
//****************************App-spoecific naming convention, binary path*********************
    // 1. set up kernel
        ...
        lud_internal = clCreateKernel(cl_NN_program, "lud_internal", &status);
        status = cl_errChk(status, (char *)"Error Creating lud_internal",true);
        if(status)exit(1);
...
//**************************************App-specific read kernel init and call ********************
  ...
  {          
    cl_int argchk;
    argchk  = clSetKernelArg(lud_internal, 0, sizeof(void *), (void*) &d_m);
    argchk |= clSetKernelArg(lud_internal, 1, sizeof(float) * BLOCK_SIZE * BLOCK_SIZE, (void*)NULL );
	argchk |= clSetKernelArg(lud_internal, 2, sizeof(float) * BLOCK_SIZE * BLOCK_SIZE, (void*)NULL );
	argchk |= clSetKernelArg(lud_internal, 3, sizeof(cl_int), (void*) &matrix_dim);
	argchk |= clSetKernelArg(lud_internal, 4, sizeof(cl_int), (void*) &i);
  ...
  //  printf("Read kernel Starts\n");
    ...
    cl_errChk(argchk,"ERROR in Setting Nearest internal args",true);
	error = clEnqueueNDRangeKernel(command_queue,lud_internal,2, NULL, global_work1, local_work1, 0, 0, &kernelEvent);
	
	cl_errChk(error,"ERROR in Executing Kernel lud_internal",true);
