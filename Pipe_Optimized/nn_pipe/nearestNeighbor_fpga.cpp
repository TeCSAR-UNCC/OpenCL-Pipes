...
//****************************App-spoecific naming convention, binary path*********************
    // 1. set up kernel
		...
		cl_NN_program = cl_compileProgram((char *)"./xclbin/krnl_nn.hw_emu.xilinx_aws-vu9p-f1-04261818_dynamic_5_0.xclbin",NULL);
		...
		NN_kernel_compute = clCreateKernel(cl_NN_program, "NearestNeighbor", &status);
...
//**************************************App-specific read kernel init and call ********************
	...
	argchk |= clSetKernelArg(NN_kernel_read, 1, sizeof(float), (void *)&numRecords);


    cl_errChk(argchk,"ERROR in Setting Nearest Neighbor_read kernel args",true);
	
	
	argchk = clSetKernelArg(NN_kernel_compute, 0, sizeof(float), (void *)&numRecords);
    argchk |= clSetKernelArg(NN_kernel_compute, 1, sizeof(float), (void *)&lat);
    argchk |= clSetKernelArg(NN_kernel_compute, 2, sizeof(float), (void *)&lng);
	...
	argchk |= clSetKernelArg(NN_kernel_wb, 1, sizeof(float), (void *)&numRecords);
